from src.models.nn.continuous_attention import ContinuousAttention
from src.models.nn.continuous_encoders import ConvEncoder
...
class AttentiveS4(nn.Module):
    ...
    def __init__(
        ...
        attn_vector_size = self.h
        attn_hidden_size = self.h
        ...
        self.attn = ContinuousAttention(
            ...
            nb_basis=128,
            penalty=0.1,
            gaussian_sigmas=[.03, .08, .3],
            wave_b=10000,
            max_seq_len=16000 if l_max == 1 else l_max,
    ...
    def forward(self, u, state=None, **kwargs): # absorbs return_output and transformer src mask
        ...
        # (B, C, H, L) -> (B*C, L, H)
        y_attn = y.reshape(-1, y.shape[2], y.shape[3]).transpose(-1, -2)
        # Apply attention
        mask = kwargs.get('attn_mask', None)
        y_attn, _ = self.attn(y_attn, y_attn, values=y_attn, mask=mask)
        # (B*C, L, H) -> (B, C, H, L)
        y_attn = y_attn.reshape(-1, self.channels, y_attn.shape[-2], y_attn.shape[-1]).transpose(-1, -2)
...
def test_state(random_init=False, **kwargs):
    ...
    N = 4
    L = 8
    s4 = AttentiveS4(H, d_state=N, l_max=L, **kwargs)
    s4.to(device)
    s4.eval()
